...
describe('PinService seedIfEmpty', () => {
  const persistence = (existing: any[] = []) => ({
    loadPins: jest.fn(async () => existing),
    savePins: jest.fn(async () => undefined),
  });

  const build = async (existing: any[] = []) => {
    const p = persistence(existing);
    const service = buildService(p);
    await service.onModuleInit();
    return { service, p };
  };

  // The code is per profile, not per run: the caller decides, so kid and
  // adult profiles can start on different ones.
  it('gives every profile the pin it was named with', async () => {
    const { service, p } = await build();

    const seeded = await service.seedIfEmpty([
      { profile: 'a', pinCode: '1111' },
      { profile: 'b', pinCode: '2222' },
    ]);

    expect(seeded).toBe(true);
    expect(service.hasPin('a')).toBe(true);
    expect(service.hasPin('b')).toBe(true);
    expect(p.savePins).toHaveBeenCalledWith([
      expect.objectContaining({ profile: 'a', pinCode: '1111' }),
      expect.objectContaining({ profile: 'b', pinCode: '2222' }),
    ]);
  });

  // Seeding must not undo a deletion: whoever removed a PIN meant it, and a
  // restart is not consent to put it back.
  it('leaves a store that already holds records alone', async () => {
    const { service, p } = await build([
      { profile: 'a', pinCode: '9999', updatedAt: 'x' },
    ]);

    const seeded = await service.seedIfEmpty([
      { profile: 'a', pinCode: '1111' },
      { profile: 'b', pinCode: '1111' },
    ]);

    expect(seeded).toBe(false);
    expect(service.hasPin('b')).toBe(false);
    expect(p.savePins).not.toHaveBeenCalled();
  });

  it('does nothing when there are no profiles to seed', async () => {
    const { service, p } = await build();

    expect(await service.seedIfEmpty([])).toBe(false);
    expect(p.savePins).not.toHaveBeenCalled();
  });
});
