...
*   **Playlist Actions & Options:**
    ...
    *   **Play All:** Non-queue playlists with tracks expose an action (`alias: "play_all"`) that triggers an `addAllToQueue` action with `{ position: "top", startPlayback: true }`. The first playlist item becomes Now Playing immediately, while the rest are added to the top of the active Queue, preserving their playlist order.
